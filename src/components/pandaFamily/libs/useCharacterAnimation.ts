...
export function useCharacterAnimation({
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      body.current?.setAttribute(
        'd',
        `M10 ${bodyBottom} ` +
          `C${10 + midShift} ${bodyMidY}, ${14 + topShift} 18, ${28 + topShift} ${BODY_TOP + 2} ` +
          `C${42 + topShift} ${BODY_TOP - 9}, ${78 + topShift} ${BODY_TOP - 9}, ${92 + topShift} ${BODY_TOP + 2} ` +
          `C${106 + topShift} 18, ${110 + midShift} ${bodyMidY}, 110 ${bodyBottom} Z`
      );
  ...
  return { body, face, ears, pupils, highlights };
}
