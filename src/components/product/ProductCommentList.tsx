...
import type { ProductComment as ProductCommentType } from '../../types/product';
...
export default function ProductCommentList({
  ...
  const handleInput = () => {
    const hasValue = (textRef.current?.value.length ?? 0) > 0;
    submitBtnRef.current?.classList.toggle(styles.active, hasValue);
  };

  const hasComments = productComments.length > 0;
  ...
  return (
    <Container>
      ...
      {hasComments ? (
        <CommentsBox>
          {productComments.map((comment) => (
            <ProductComment productComment={comment} key={comment.id} />
          ))}
        </CommentsBox>
      ) : (
        <EmptyCommentsImg />
      ...
      {hasMoreComments && (
        <LoadMoreCommentsButton isLoading={isLoading} onLoadMore={onLoadMore} />
      )}
    </Container>
  );
}
